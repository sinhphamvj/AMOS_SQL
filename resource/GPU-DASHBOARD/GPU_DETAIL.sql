SELECT
    ...
    TO_CHAR(TRUNC((MIN(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.start_date * 10000 + time_captured.start_time  END) % 10000) / 60), 'FM00') || ':' || 
    TO_CHAR(MOD((MIN(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.start_date * 10000 + time_captured.start_time  END) % 10000), 60), 'FM00') AS gpu_start_time,
    ...
    TO_CHAR(TRUNC((MAX(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.end_date * 10000 + time_captured.end_time    END) % 10000) / 60), 'FM00') || ':' || 
    TO_CHAR(MOD((MAX(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.end_date * 10000 + time_captured.end_time    END) % 10000), 60), 'FM00')   AS gpu_end_time,
    TO_CHAR(TRUNC(SUM(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.duration    END) / 60), 'FM00') || ':' || 
    TO_CHAR(MOD(SUM(CASE WHEN rotables.partno IS NOT NULL OR rotables.serialno IS NOT NULL
             THEN time_captured.duration    END), 60), 'FM00')                   AS gpu_duration,
