...
FROM
    (
    SELECT DISTINCT ON (d.day, s.user_sign)
        d.day,
        s.user_sign,
        CASE
            WHEN spq.pqs_type_no_i = 1378 AND s.skill_shop NOT LIKE '%B%' THEN 'A'
    ...
    JOIN (
        SELECT DISTINCT s.user_sign, s.employee_no_i, s.skill_shop, sua.end_date
        FROM sign s
        JOIN sp_user_availability sua ON s.user_sign = sua.user_sign
        JOIN sp_shift ON sp_shift.shift_id = sua.shift_id
        WHERE  s.status = 0 AND sp_shift.location = 'SGN' AND sp_shift.shift_type = 'LINE'
        AND sp_shift.shift_group_no_i IN (3702,903,904,905,906,907,1110,3902,1114)
          AND (
             (
    ...
    LEFT JOIN staff_pqs_qualification spq ON s.employee_no_i = spq.employee_no_i AND (DATE '1971-12-31'+spq.expiry_date) >= d.day
    ORDER BY d.day, s.user_sign,
        CASE
            WHEN spq.pqs_type_no_i = 1378 AND s.skill_shop NOT LIKE '%B%' THEN 1
            WHEN spq.pqs_type_no_i = 1379 THEN 2
            WHEN spq.pqs_type_no_i IN (1380, 2771) THEN 3
            ELSE 99
        END
) dc
GROUP BY
