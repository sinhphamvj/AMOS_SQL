...
FROM (
    ...
    AND (
        (
            (current_timestamp + INTERVAL '7 hours')::time BETWEEN '07:00:00' AND '19:00:00'
            AND (entry_type IN ('B1', 'B11', 'B12', 'B16', 'B20', 'B21', 'B3', 'B5', 'B7','EC','B1_O','HC','OT_M')
                OR (entry_type IN('OT','B18') AND start_time < 750)
            )
        )OR (
            (current_timestamp + INTERVAL '7 hours')::time NOT BETWEEN '07:00:00' AND '19:00:00'
            AND (entry_type IN ('B10', 'B13', 'B19', 'B2', 'B22', 'B4', 'B6', 'B8', 'E','B2_O','OT_N','K2')
                OR (entry_type IN ('OT','B18') AND start_time > 750)
            )
        )
    )
    AND sign.status <> 9
    AND sp_shift.location = 'PQC'
