SELECT 
    ...
    SUM(CASE WHEN "TASK_TYPE" = 'ASSIGNED' THEN "TASK_COUNT" ELSE 0 END) AS "ASSINGED_STRUCT_TOTAL",
    
    -- Tổng số nhiệm vụ Deferred (DEFERRED với prio = 'M')
    SUM(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END) AS "DEFERRED_STRUCT_TOTAL",
    
    -- Tổng số nhiệm vụ Closed (CLOSED với prio = 'M')
    (MAX(CASE WHEN "TASK_TYPE" = 'CLOSED' THEN "TASK_COUNT" ELSE 0 END)-
     MAX(CASE WHEN "TASK_TYPE" = 'JC_NOT_PERFORMED' THEN "TASK_COUNT" ELSE 0 END)) AS "CLOSED_STRUCT_TOTAL",
    
    -- Tổng số nhiệm vụ (Mandatory + Deferred)
    (MAX(CASE WHEN "TASK_TYPE" = 'CLOSED' THEN "TASK_COUNT" ELSE 0 END) +
     MAX(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END)-
     MAX(CASE WHEN "TASK_TYPE" = 'JC_NOT_PERFORMED' THEN "TASK_COUNT" ELSE 0 END)) AS "TOTAL_STRUCT_TASK"
...
FROM (
    ...
    -- Query cho CLOSED STRUCTURE TOTAL
