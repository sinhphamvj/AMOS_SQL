SELECT 
    ...
    SUM(CASE WHEN "TASK_TYPE" = 'ASSIGNED' THEN "TASK_COUNT" ELSE 0 END) AS "ASSINGED_WEEKLY_TOTAL",
    
    -- Tổng số nhiệm vụ Deferred (DEFERRED với prio = 'M')
    SUM(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END) AS "DEFERRED_WEEKLY_TOTAL",
    
    -- Tổng số nhiệm vụ Closed (CLOSED với prio = 'M')
    SUM(CASE WHEN "TASK_TYPE" = 'CLOSED' THEN "TASK_COUNT" ELSE 0 END) AS "CLOSED_WEEKLY_TOTAL",
    
    -- Tổng số nhiệm vụ (Mandatory + Deferred)
    (SUM(CASE WHEN "TASK_TYPE" = 'CLOSED' THEN "TASK_COUNT" ELSE 0 END) +
     SUM(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END)) AS "TOTAL_WEEKLY_TASK"
...
FROM (
    ...
    AND wo_header.event_type = 'C'
    AND wo_text_description.header LIKE '%WEEKLY%'
