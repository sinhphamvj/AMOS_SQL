SELECT 
    ...
    SUM(CASE WHEN "TASK_TYPE" = 'ASSIGNED' THEN "TASK_COUNT" ELSE 0 END) AS "ASSINGED_ENGR_TOTAL",
    
    -- Tổng số nhiệm vụ BSI Deferred 
    SUM(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END) AS "DEFERRED_ENGR_TOTAL",
    
    -- Tổng số nhiệm vụ BSI Closed 
    SUM(CASE WHEN "TASK_TYPE" = 'CLOSED' THEN "TASK_COUNT" ELSE 0 END) AS "CLOSED_ENGR_TOTAL",
    
    -- Tổng số nhiệm vụ (Mandatory + Deferred)
    (SUM(CASE WHEN "TASK_TYPE" = 'ASSIGNED' THEN "TASK_COUNT" ELSE 0 END) +
     SUM(CASE WHEN "TASK_TYPE" = 'DEFERRED' THEN "TASK_COUNT" ELSE 0 END)) AS "TOTAL_ENGR_TASK"
...
FROM (
    ...
    LEFT JOIN workstep_classification ON workstep_link.descno_i = workstep_classification.descno_i
    ...
    AND (workstep_classification.power_run = 'Y' OR workstep_classification.idle_run = 'Y')

    UNION ALL
    
    -- Query cho DEFERRED_ENGR_TOTAL 
