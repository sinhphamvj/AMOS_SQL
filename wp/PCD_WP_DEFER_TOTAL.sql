SELECT DISTINCT ON (wp_content.event_perfno_i)     ROW_NUMBER() OVER (
          PARTITION BY wp_header.ac_registr         
       ORDER BY wp_header.ac_registr 
) AS "ITEM",
...
MAX(
   CASE WHEN moc_daily_records.event_type = 'AOG' THEN 1 ELSE 0 END
   ) AS "AOG",
MAX(
   CASE WHEN moc_daily_records.event_type = 'ST_MT' THEN 1 ELSE 0 END
   ) AS "STORAGE",
MAX(
   CASE WHEN moc_daily_records.event_type = 'PK_MT' THEN 1 ELSE 0 END
   ) AS "PARKING",
...
LEFT JOIN moc_daily_records ON wp_header.ac_registr = moc_daily_records.ac_registr
   ...
   AND moc_daily_records.event_type IN (
      'AOG',
      'ST_MT',
      'PK_MT'
   )
LEFT JOIN maint_event_ext_reason ON wp_content.reason = maint_event_ext_reason.reason
WHERE wp_content.wpno_i IN (@PCD_WP_MAIN.wpno_i@)
AND wo_header.state IN (
   'C',
   'O'
   )
