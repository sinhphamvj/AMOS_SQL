...
RIGHT JOIN (
    ...
    FROM 
        sign s   
    LEFT JOIN 
        staff_pqs_qualification spq ON spq.employee_no_i = s.employee_no_i
    LEFT JOIN 
    ...
    WHERE  
        s.employee_no_i NOT IN (
            ...
            LEFT JOIN  
                staff_pqs_type ON staff_pqs_type.pqs_type_no_i = spq.pqs_type_no_i 
            LEFT JOIN 
                staff_pqs_class ON staff_pqs_type.pqs_class_no_i = staff_pqs_class.pqs_class_no_i
            LEFT JOIN (
                SELECT 
                    s.employee_no
                FROM 
            ...
            WHERE  
                ...
                AND staff_pqs_type.type_label = 'TVJ'
...
LEFT JOIN 
    sign ON sign.employee_no_i = valid_staff.employee_no_i
LEFT JOIN (
    SELECT 
        sign.employee_no_i, 
        TO_CHAR(DATE '1971-12-31' + spq.expiry_date, 'DD Mon YYYY') AS aml_expiry_date, 
        spq.ac_type
    FROM 
        sign
    LEFT JOIN 
        staff_pqs_qualification spq ON spq.employee_no_i = sign.employee_no_i
    LEFT JOIN  
        staff_pqs_type ON staff_pqs_type.pqs_type_no_i = spq.pqs_type_no_i 
    WHERE 
...
WHERE 
    /*staff_pqs_type.type_description ='VJC AUTH'
      AND*/ 
    valid_staff.auth_number LIKE '%VJC.CRS%'
    AND sign.status = 0
ORDER BY 
    valid_staff.auth_number