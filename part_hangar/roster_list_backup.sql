...
FROM staff_pqs_type
RIGHT JOIN (SELECT DISTINCT s.firstname,
    			        s.lastname,
              		              s.employee_no,
			            s.employee_no_i,
     			       s.auth_number,
			            ...
			            TO_CHAR(DATE '1971-12-31'+spq.expiry_date,'DD Mon YYYY') AS expiry_date,
                                                            spq.special_text_1, 
			            CASE WHEN s.jobcode='B1LE' THEN spq.special_text_2
					WHEN s.jobcode='B2LE' THEN spq.special_text_2
					ELSE ''
			            END AS system
			           /* spq.special_text_2 AS system*/
                         FROM sign  s   
                         ...
                         WHERE  s.employee_no_i NOT IN (SELECT s.employee_no_i
					           ...
					           LEFT JOIN (SELECT s.employee_no
								...
								WHERE staff_pqs_type.type_label = 'SPS' AND spq.status=0) sus ON sus.employee_no=s.employee_no
					           WHERE  spq.expiry_date <   CURRENT_DATE-'1971-12-31'
					          	AND  spq.expiry_date IS NOT NULL
					          	AND (s.lastname LIKE '%VJC.CRS%')  
     					                     AND staff_pqs_type.type_label NOT IN  ('PROC TVJ','AVSEC-MANAGER', 'SMS-MANAGER', 'PDA', 'SMS-GSE', 'GSE - QUALIFIED COURSE') 	
     					                     AND staff_pqs_class.pqs_class_id <> 'RECURRENT'  
						     AND staff_pqs_class.pqs_class_id <> 'GSE-GO'
						          AND spq.status=0) /*to exclude staff with expired qualification (exclude TVJ PROC recurrent*/
		AND  s.employee_no_i  NOT IN (SELECT s.employee_no_i
								FROM sign s
								RIGHT JOIN staff_pqs_qualification spq ON spq.employee_no_i=s.employee_no_i 
								RIGHT JOIN  staff_pqs_type ON staff_pqs_type.pqs_type_no_i=spq.pqs_type_no_i 
								RIGHT JOIN staff_pqs_class ON staff_pqs_type.pqs_class_no_i=staff_pqs_class.pqs_class_no_i
								WHERE staff_pqs_type.type_label = 'SPS' AND spq.status=0)/*to exclude staff with superseded authorization*/
		AND staff_pqs_type.type_description ='VJC AUTH')  valid_staff ON valid_staff.pqs_type_no_i=staff_pqs_type.pqs_type_no_i
LEFT JOIN sign ON sign.employee_no_i=valid_staff.employee_no_i
LEFT JOIN (SELECT sign.employee_no_i, TO_CHAR(DATE '1971-12-31'+spq.expiry_date,'DD Mon YYYY') AS aml_expiry_date, spq.ac_type
		FROM sign
		LEFT JOIN staff_pqs_qualification spq ON spq.employee_no_i=sign.employee_no_i
		 LEFT JOIN  staff_pqs_type ON staff_pqs_type.pqs_type_no_i=spq.pqs_type_no_i 
		WHERE staff_pqs_type.type_label='AMT') aml ON aml.employee_no_i=valid_staff.employee_no_i AND aml.ac_type=valid_staff.ac_type
WHERE /*staff_pqs_type.type_description ='VJC AUTH'
      AND*/ valid_staff.lastname LIKE '%VJC.CRS%'
	AND sign.status=0
ORDER BY valid_staff.lastname