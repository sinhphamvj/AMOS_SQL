SELECT
    ...
    spt.type_label,
    CASE
        WHEN s.main_scope LIKE 'A%' THEN 'CAT II/III, RVSM, PBN'
        WHEN s.main_scope LIKE 'B%' AND spq.notes LIKE '%8c%' THEN 'CAT II/III, RVSM, PBN,RII/DI'   
        ELSE 'CAT II/III, RVSM, PBN'
    END                                                             AS SPECIAL_OPERATION
FROM
    sign s
    /* --- Qualification & Type: chỉ lấy TVJ --- */
    ...
    /* --- Loại trừ nhân viên có TVJ qualification hết hạn (class CUS_AUTH) --- */
    LEFT JOIN (
        ...
        WHERE spq2.expiry_date < CURRENT_DATE - DATE '1971-12-31'
          ...
          AND spq2.status = 0
          AND spt2.type_label = 'TVJ'
          AND spc2.pqs_class_id = 'CUS_AUTH'
    ) expired ON expired.employee_no_i = s.employee_no_i
    /* --- AML expiry date (pqs_type_no_i = 1376, ac_type A32%) --- */
    LEFT JOIN (
        SELECT
            spq3.employee_no_i,
        ...
        WHERE spq3.pqs_type_no_i = 1376
          AND spq3.ac_type LIKE 'A32%'
